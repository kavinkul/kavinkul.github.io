...
function Graph({ size, chosenValues }) {
  ...
  useEffect(() => {
    const updatedEdgesDrawn = () =>
      objectFilter(edgesDrawn, (obj) => obj.index1 < size && obj.index2 < size);
  ...
  const handleVertexClick = (vertex) => {
    if (selectedVertex === -1) {
      ...
      if (selectedVertex !== vertex) {
        const newPairKey = generateUniquePairKey(selectedVertex, vertex);
        const updateEdgesDrawn = { ...edgesDrawn };
        if (
          updateEdgesDrawn[newPairKey] !== null &&
          updateEdgesDrawn[newPairKey] !== undefined &&
          updateEdgesDrawn[newPairKey].isDrawn
        ) {
